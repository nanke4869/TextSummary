...
df = pd.read_csv("SUMM_train.csv")
input_test = pd.read_csv("SUMM_test.csv")
train_df = df.drop('id', axis=1)
test_df = train_df.iloc[0:20]
id = input_test['id']
test = input_test.drop('id', axis=1)

log = open("logging.txt", "a+", encoding='utf-8')
'''
构建词典
chars
id2char
...
if os.path.exists('vocab.json'):
    ...
    chars = {}
    for summarization, text in tqdm(train_df.values):
        for w in text:
...
def print_sentence(s, model):
    # 查看某个具体句子实际效果
    print(gen_sent(s, model))
    log.write(str(gen_sent(s, model)))
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch >= 7:
            ...
            with open("submission"+str(epoch)+".json", "w", encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    with open("../ml_contest/1submission.json", "w", encoding='utf-8') as f:
        json.dump(pred, f, indent=2, ensure_ascii=False)